...
ItemWidget *ItemDelegate::cache(const QModelIndex &index)
{
    ...
    if (w == nullptr) {
        ...
        emit itemWidgetCreated(PersistentItemSelection(m_view->tabName(), data, w->widget()));
    }

    return w;
}

void ItemDelegate::updateCache(const QObject &widget, const QVariantMap &data)
{
    ...
    if ( data.isEmpty() ) {
        const auto ww = qobject_cast<const QWidget*>(&widget);
        if ( ww->isVisible() ) {
            const auto oldData = m_view->itemData(index);
            emit itemWidgetCreated(PersistentItemSelection(m_view->tabName(), oldData, ww));
        } else {
